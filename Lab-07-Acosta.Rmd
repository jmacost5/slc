...
```{r}
row.ids <- sapply( slot( sea, "polygons" ), function(x) slot( x, "ID" ) )
row.names( sea ) <- row.ids
```

```{r}
slc <- spTransform( sea, CRS("+proj=longlat +datum=WGS84") )
geojson_write( sad, file="slc_dorling.geojson", geometry="polygon" )
```
